...
| SCHEMA Tab + Data Architecture Registry (Phase 6) | T2 | ✅ READY (commit pending user push) | 2026-05-15 | New `lib/architecture/schema-map.ts` registry — single source of truth for the data architecture (66 nodes: foundation 16, sources 21, integration 5, storage 4, api 7, views 12, bi 4). Typed `ArchNode` schema with `Layer` × `Status` × `tech` × `consumes/exposes` × `fields[]` × `nextStep` × `ownerPhase`. New `lib/architecture/helpers.ts` exposing `getNode`, `getNodesByLayer`, `getMaturityCell`, `getStatusColor`, `getFlowEdges`, `getErdEdges`. New 12th tab **SCHEMA** with 4 modes — **Stack** (7 frosted swim lanes, hover dims unrelated cards, click opens detail panel), **ERD** (ReactFlow dagre TB, `EntityCardNode` 220 px rectangular w/ field list + 🔑PK / → FK markers + cardinality 1:1 / 1:N labels), **Flow** (ReactFlow dagre LR, sources→bi, edge color & width by status, implemented edges animate), **Maturity** (CSS Grid matrix Entity × {Type, Mock, Storage, API, UI, BI, Live}, color per cell via best-status lookup, tooltip with nextStep). `ArchNodeDetailPanel` (right-side glass card, description + fields table + consumes/exposes/consumedBy clickable links + filePath + nextStep). New 11th tab **DATA GRID** built from existing GRAFO infra (ReactFlow + dagre LR rendering ontology + instances) since prompt assumed it shipped in Phase 5.1 — it did not. `NavViewId` extended w/ `data-grid` and `schema`; TopNavBar order now `Comando · Operador · Cuentas · Calidad · Cadena de Frío · Radar · Social · SIM · DATA GRID · Grafo · SCHEMA · Config` (12 pills) with tightened density (padding 0.375rem 0.75rem · font-size 0.7rem · gap 0.15rem; at ≤1280 px 0.65rem). Detail-panel suppression extended to schema & data-grid. CLAUDE.md gained a new rule: "Any new entity, source, endpoint, storage target, or BI integration MUST be registered in `lib/architecture/schema-map.ts` in the same commit it's introduced." AUDIT artifact at `prompts/phase6/AUDIT.md`. tsc clean, npm run build ✅ green. **NOT yet pushed/deployed** — awaiting user confirmation per phase agreement (Local commit only). |
...
| **Phase 7 — Storage Foundation** | TBD | 📋 UPCOMING | — | Supabase Postgres + Vercel Blob, mirror ontology to schema, entity CRUD endpoints, persist Config/Operator threads. Registry-driven planning at SCHEMA → Stack → Storage. |
| **Phase 8 — IoT & Cold-chain Live** | TBD | 📋 UPCOMING | — | TimescaleDB extension, reefer-logger webhooks, `/api/ingest/reefer`, AIS shipping tracker, Weather API. Replaces mock-coldchain + mock-temperaturas. |
| **Phase 9 — ERP & CRM Sync** | TBD | 📋 UPCOMING | — | Nisira ERP REST integration, CRM postventa, portal de reclamos cliente, Neo4j Aura for the real graph, GraphRAG indexer over real data. |
| **Phase 10 — BI & External Agents** | TBD | 📋 UPCOMING | — | Power BI dataset + customer portal, Looker/Metabase secondary, MCP server for agent read access. Closes the planning chain in `bi` layer. |
